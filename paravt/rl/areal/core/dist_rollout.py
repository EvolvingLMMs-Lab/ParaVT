...
def redistribute_trajectories(
    ...
    # Remove pad positions from each trajectory (fix: was discarding return value)
    for i, d in enumerate(all_data):
        old_seq_len = d["attention_mask"].shape[-1] if "attention_mask" in d else None
        all_data[i] = _remove_padding_from_trajectory(d)
        new_seq_len = all_data[i]["attention_mask"].shape[-1] if "attention_mask" in all_data[i] else None
        if old_seq_len is not None and new_seq_len is not None and old_seq_len != new_seq_len:
            import logging as _logging
            _logging.getLogger("rmpad").info(
                f"rmpad: trajectory {i} seq_len {old_seq_len} -> {new_seq_len} "
                f"(removed {old_seq_len - new_seq_len} padding tokens, {(old_seq_len - new_seq_len) / old_seq_len * 100:.1f}%)"
            )
