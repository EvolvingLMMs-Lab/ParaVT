...
def is_subagent_trajectory_key(key: str) -> bool:
    return key.startswith("subagent_trajectories")
...
def concat_padded_tensors(
    ...
    subagent_keys = {
        key for key in tensor_dicts[0].keys() if is_subagent_trajectory_key(key)
    }
    for subagent_key in subagent_keys:
        concat_subagent = []
        for td in tensor_dicts:
            if subagent_key in td and len(td[subagent_key]) > 0:
                concat_subagent.extend(td[subagent_key])
        result[subagent_key] = concat_subagent

    # Process each key
    for key in tensor_dicts[0].keys():
        tensors_to_concat = []
        if is_multi_modal_key(key) or is_subagent_trajectory_key(key):
            continue
...
def all_gather_tensor_container(data, group=None) -> list:
    ...
    if isinstance(data, list):
        ...
        if len(set(lengths)) == 1:
            ...
            padded_length = max(lengths) - len(data)
            global_padded_length = [None for _ in range(world_size)]
            dist.all_gather_object(global_padded_length, padded_length, group=group)
            padded_data = data + [deepcopy(data[0]) for _ in range(padded_length)]
            padded_data = [
                all_gather_tensor_container(d, group=group) for d in padded_data
            ]

            new_padded_data = []
            for pad_len, padded_d in zip(global_padded_length, padded_data):
                new_padded_data.append(padded_d[: len(data)])

            return list(zip(*padded_data))
