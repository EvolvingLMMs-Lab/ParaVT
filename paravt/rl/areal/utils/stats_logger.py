...
class StatsLogger:
    ...
    def init(self):
        ...
        # Use pre-created run ID from launch.sh if available (avoids init timeout
        # during heavy GPU loading), otherwise generate one
        run_id = os.environ.get("WANDB_RUN_ID") or \
            f"{self.config.experiment_name}_{self.config.trial_name}_{suffix}"

        _wandb_init_kwargs = dict(
        ...
        for _attempt in range(3):
            try:
                ...
                if _attempt < 2:
                    logger.warning(f"wandb.init attempt {_attempt+1} failed: {e}, retrying in 10s...")
                    time.sleep(10)
                else:
                    logger.error(
                        f"wandb.init failed after 3 attempts: {e}. "
